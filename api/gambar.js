...
export default async function handler(req, res) {
  ...
  // NOTE: jangan destrukturisasi "sharp" langsung karena akan shadow import
  const { key, url, h, w, q, fit, format, sharpLevel, text } = req.query;
  // ambil query 'sharp' tapi simpan ke variabel doSharp untuk menghindari shadowing
  ...
  if (!w && !h && !q && !format && !doSharp && !fit && !text) {
    res.setHeader('Content-Type', contentType);
    res.setHeader('Content-Length', imageData.length);
    res.setHeader('Cache-Control', 'public, max-age=86400, s-maxage=31536000, immutable');
    res.setHeader('X-Rate-Limit-Remaining', rateLimitResult.remaining.toString());
