...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Jika ada parameter w atau h, resize pakai Sharp
    const width = w ? parseInt(w, 10) : null;
    const height = h ? parseInt(h, 10) : null;

    if (width || height) {
      const validFit = ['cover', 'contain', 'fill', 'inside', 'outside'].includes(fit) ? fit : 'cover';
    ...
    // Tanpa resize, kirim gambar asli seperti sebelumnya
